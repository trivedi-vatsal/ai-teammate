...
async function checkAzureConfig() {
  ...
  if (result.error) {
    console.log(
      "📝 No .env file found, checking system environment variables...\n"
    );
  } else {
  ...
  console.log(
    `✅ AZURE_OPENAI_MODEL_NAME: ${modelName ? "✓ Set" : "❌ Missing"}\n`
  );
  ...
  try {
      ...
      [
        {
          role: "user",
          content:
            "Hello! This is a test message to verify Azure OpenAI connection.",
        },
      ],
      {
        maxTokens: 50,
    ...
    console.log(
      "🎉 All checks passed! Azure OpenAI configuration is working correctly."
    );
  } catch (error) {
